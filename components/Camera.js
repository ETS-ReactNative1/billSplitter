...
export default function App({ navigation, route }) {
  ...
  const fetchOCRData = async (capturedImage) => {
    ...
    try {
      ...
      setIsLoading(false);
      console.log(
        '👋 Is Error Processing from ocr space fetch ------>',
        ocr.IsErroredOnProcessing,
      );
      console.log(
        '👋 Error Message from ocr space fetch ------>',
        ocr.ErrorMessage,
      );
    ...
    <View flex={1}>
      ...
      >
        <Pressable
          ...
          p={2}
          onPress={() =>
            navigation.navigate('BillScreen', { payer, billName })
          }
        >
