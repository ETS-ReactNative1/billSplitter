...
import { Feather, Entypo, FontAwesome, SimpleLineIcons } from "@expo/vector-icons";
...
export default function FinalizedBill({ navigation, route }) {
	...
	const finalizeAlert = () => {
		Alert.alert("Confirm before submitting!", "Does everything look good?", [
			{
				text: "Looks Good!",
				onPress: () => {
		...
		<Center w="100%">
			<Box maxW="400" w="100%">
				...
				<VStack space={2}>
					...
					<Divider w="87%" alignSelf="center" backgroundColor="black" />
					{/* Subtotal */}
					...
					{bill.totals.map((total, index) => (
						<HStack key={index} w="100%" justifyContent="center">
							<HStack w="45%" justifyContent="flex-start">
								...
								<Text mx="2">{bill.payer === total.assignee ? "paid for themself" : `owes ${bill.payer}`}</Text>
							</HStack>
